...
// 订单状态
const (
	OrderStatusPending          = "pending"            // 待审核
	OrderStatusPaid             = "paid"               // 已付款（即时购买）
	OrderStatusApproved         = "approved"           // 已审核（审批流程）
	OrderStatusRejected         = "rejected"           // 已拒绝
	OrderStatusCompleted        = "completed"          // 已完成
	OrderStatusCancelled        = "cancelled"          // 已取消
	OrderStatusInsufficientFund = "insufficient_funds" // 余额不足（审核时）
)
...
type ShopProduct struct {
	...
	Stock        int     `gorm:"default:-1"                      json:"stock"`         // -1 = 无库存限制
...
type ShopOrder struct {
	...
	ShippingStatus string     `gorm:"size:20;default:''"             json:"shipping_status"` // "" / pending / shipped
}
...
type ShopLotteryActivity struct {
	...
	StartAt     *time.Time         `json:"start_at"`                                      // nil = 不限
	EndAt       *time.Time         `json:"end_at"`                                        // nil = 不限
...
type ShopLotteryPrize struct {
	...
	TotalStock        int    `gorm:"not null;default:0"      json:"total_stock"`        // 库存总量，0=不限
	DrawnCount        int    `gorm:"not null;default:0"      json:"drawn_count"`        // 已抽出数量
	NeedDelivery      bool   `gorm:"not null;default:true"   json:"need_delivery"`      // 是否需要发放
}
